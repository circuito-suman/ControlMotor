    ...
    : QGroupBox(axis.toUpper() + " Axis", parent), axisName(axis)
{
    ...
    posLabel = new QLabel("0.00 mm"); posLabel->setAlignment(Qt::AlignCenter);
    posLabel->setStyleSheet("font-weight:bold; font-size:15px; color:#147;");
    base->addWidget(posLabel);

    QHBoxLayout* stepRow = new QHBoxLayout; stepRow->setSpacing(10);
    homeBtn = new QPushButton("Home");
    moveMinusBtn = new QPushButton("-");
    stepSpin = new QDoubleSpinBox; stepSpin->setRange(0.1,100); stepSpin->setValue(axis=="z"?5.0:10.0);
    stepSpin->setSuffix(" mm");
    movePlusBtn = new QPushButton("+");
    stepRow->addWidget(homeBtn); stepRow->addWidget(moveMinusBtn); stepRow->addWidget(stepSpin); stepRow->addWidget(movePlusBtn);
    base->addLayout(stepRow);

    QHBoxLayout* gotoRow = new QHBoxLayout; gotoRow->setSpacing(10);
    goLabel = new QLabel("Goto:"); goSpin = new QDoubleSpinBox; goSpin->setRange(-999,999);
    goBtn = new QPushButton("Go To");
    gotoRow->addWidget(goLabel); gotoRow->addWidget(goSpin); gotoRow->addWidget(goBtn);
    base->addLayout(gotoRow);

    QHBoxLayout* markRow = new QHBoxLayout;
    markMinBtn = new QPushButton("Min"); markMidBtn = new QPushButton("Mid"); markMaxBtn = new QPushButton("Max");
    markRow->addWidget(markMinBtn); markRow->addWidget(markMidBtn); markRow->addWidget(markMaxBtn);
    base->addLayout(markRow);

    base->setSpacing(8); setLayout(base);
...
void AxisControlWidget::setEnabledAll(bool enabled) {
    homeBtn->setEnabled(enabled); moveMinusBtn->setEnabled(enabled); movePlusBtn->setEnabled(enabled); goBtn->setEnabled(enabled);
    stepSpin->setEnabled(enabled); goSpin->setEnabled(enabled); markMinBtn->setEnabled(enabled); markMidBtn->setEnabled(enabled); markMaxBtn->setEnabled(enabled);
}
...
void MainWindow::buildUI() {
    ...
    rootLayout->setSpacing(16); rootLayout->setContentsMargins(18,12,18,12);
    ...
    connRow->addWidget(portLbl); connRow->addWidget(portCombo,2); connRow->addWidget(refreshBtn);
    connRow->addWidget(connectBtn); connRow->addWidget(disconnectBtn); connRow->addWidget(statusLabel);
    ...
    statusLog = new QTextEdit; statusLog->setReadOnly(true); statusLog->setMaximumHeight(70);
    botRow->addWidget(estopBtn,0); botRow->addWidget(statusLog,2);
...
void MainWindow::connectPort() {
    ...
    for (auto* aw : axisControls) aw->setEnabledAll(true);
    connectBtn->setEnabled(false); disconnectBtn->setEnabled(true); connected = true;
...
void MainWindow::disconnectPort() {
    ...
    for (auto* aw : axisControls) aw->setEnabledAll(false);
    connectBtn->setEnabled(true); disconnectBtn->setEnabled(false); connected = false;
    pollTimer->stop();
}
...
void MainWindow::directionalClicked() {
    if (!connected) { updateStatus("Not connected."); return; }
    QPushButton* btn = qobject_cast<QPushButton*>(sender());
    QString dir = btn->text();
    double step = 10.0;
    double dx=0, dy=0;
    if (dir.contains("N")) dy += step;
    if (dir.contains("S")) dy -= step;
    if (dir.contains("E")) dx += step;
    if (dir.contains("W")) dx -= step;
    if (dir == "Home") {
        serial->write("G28\n");
...
void MainWindow::markPosition() {
    ...
    double val = 0.0;
    if (aw->axisName == "x") val = lastPosX;
    else if (aw->axisName == "y") val = lastPosY;
    else val = lastPosZ;

    AxisMeasurement* m = measurement(aw->axisName);
    if (type == "min")      m->min = val;
    else if (type == "mid") m->mid = val;
    else if (type == "max") m->max = val;
